...
void llama_context::opt_init(struct llama_model * model, struct llama_opt_params lopt_params) {
    ...
    // then reserve enough capacity for the expanded backward graph.
    {
        ...
        // The backward graph can add more than four nodes per forward node for QAT.
        const int64_t inflated   = (int64_t)std::max<uint32_t>(train_fwd_nodes, 1024u) * 8;
        ...
        sched_need_reserve = false;
        LLAMA_LOG_INFO("%s: training graph capacity = %lld (train_fwd_nodes=%u x8)\n",
