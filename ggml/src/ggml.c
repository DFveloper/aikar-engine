...
size_t ggml_row_size(enum ggml_type type, int64_t ne) {
    ...
    return ggml_type_size(type)*ne/ggml_blck_size(type) + type_traits[type].row_meta_size;
}
...
bool ggml_is_contiguously_allocated(const struct ggml_tensor * tensor) {
    return ggml_nbytes(tensor) == ggml_row_size(tensor->type, tensor->ne[0])*ggml_nrows(tensor);
}
...
void ggml_set_fused_backward(struct ggml_context * ctx, bool enabled) {
    ctx->fused_backward = enabled;
}
...
struct ggml_tensor * ggml_rms_norm_mul_back(
        struct ggml_context * ctx,
        struct ggml_tensor  * grad,
        struct ggml_tensor  * input,
        struct ggml_tensor  * scale,
        float                 eps) {
    GGML_ASSERT(grad->type == GGML_TYPE_F32 && input->type == GGML_TYPE_F32 && scale->type == GGML_TYPE_F32);
    GGML_ASSERT(ggml_are_same_shape(grad, input));
    GGML_ASSERT(scale->ne[0] == input->ne[0] && ggml_nrows(scale) == 1);
    struct ggml_tensor * result = ggml_rms_norm_back(ctx, grad, input, eps);
    result->src[2] = scale;
    return result;
}
...
struct ggml_tensor * ggml_out_prod(
        ...
        struct ggml_tensor  * a,
        struct ggml_tensor  * b) {
    if (a->type != GGML_TYPE_F32 && !ggml_is_quantized(a->type)) {
    ...
    if (ggml_is_quantized(a->type) && a->buffer) {
        ggml_backend_buffer_type_t buft = ggml_backend_buffer_get_type(a->buffer);
        ggml_backend_dev_t dev = ggml_backend_buft_get_device(buft);
        if (dev && !ggml_backend_dev_supports_op(dev, result)) {
            result->src[0] = ggml_cast(ctx, a, GGML_TYPE_F32);
        }
    }
...
struct ggml_tensor * ggml_glu_back(
        ...
        enum ggml_glu_op      op) {
    GGML_ASSERT(op == GGML_GLU_OP_SWIGLU || op == GGML_GLU_OP_GEGLU);
    GGML_ASSERT(ggml_are_same_shape(grad, gate));
    GGML_ASSERT(ggml_are_same_shape(gate, up));
    GGML_ASSERT(grad->type == GGML_TYPE_F32 && gate->type == GGML_TYPE_F32 && up->type == GGML_TYPE_F32);

    grad = ggml_is_contiguous(grad) ? grad : ggml_cont(ctx, grad);
    gate = ggml_is_contiguous(gate) ? gate : ggml_cont(ctx, gate);
    up   = ggml_is_contiguous(up)   ? up   : ggml_cont(ctx, up);

    const int64_t ne[4] = { 2*gate->ne[0], gate->ne[1], gate->ne[2], gate->ne[3] };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);
    ggml_set_op_params_i32(result, 0, op);
...
struct ggml_tensor * ggml_acc_qlion_qat(
        struct ggml_context * ctx,
    ...
    result->op = GGML_OP_ACC_QLION_QAT;
...
struct ggml_tensor * ggml_acc_qlion_qat_tied(
        struct ggml_context * ctx,
        struct ggml_tensor  * accumulator,
    ...
    GGML_ASSERT(accumulator->ne[0] == dense_a->ne[0]);
    GGML_ASSERT(accumulator->ne[1] == dense_b->ne[0]);
    GGML_ASSERT(dense_a->ne[1] == dense_b->ne[1]);
    GGML_ASSERT(sparse_grad->ne[0] == accumulator->ne[0]);
    GGML_ASSERT(sparse_grad->ne[1] == ggml_nelements(sparse_ids));
    GGML_ASSERT(ggml_is_contiguous(accumulator) && ggml_is_contiguous(dense_a));
    GGML_ASSERT(ggml_is_contiguous(dense_b) && ggml_is_contiguous(sparse_grad));
...
struct ggml_tensor * ggml_opt_step_qlion_qat_rows(
        struct ggml_context * ctx,
        struct ggml_tensor  * weight,
        struct ggml_tensor  * grad,
        struct ggml_tensor  * ids,
    ...
    struct ggml_tensor * result = ggml_view_tensor(ctx, weight);
    result->op = GGML_OP_OPT_STEP_QLION_QAT_ROWS;
    result->src[0] = weight;
    result->src[1] = grad;
    result->src[2] = ids;
...
struct ggml_tensor * ggml_turbo_wht(
    ...
    result->src[0] = a;
    ggml_set_op_params_i32(result, 0, inverse ? 1 : 0);
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_CUMSUM: {
            if (src0_needs_grads) {
                ...
                struct ggml_tensor * ones = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n, n);
                ones = ggml_fill(ctx, ones, 1.0f);
                ones = ggml_tri(ctx, ones, GGML_TRI_TYPE_UPPER_DIAG);

                struct ggml_tensor * grad_2d = ggml_reshape_2d(ctx, ggml_cont(ctx, grad), n, ggml_nelements(grad)/n);
                struct ggml_tensor * back = ggml_mul_mat(ctx, ones, grad_2d);
                ggml_add_or_set(ctx, cgraph, isrc0, ggml_reshape(ctx, back, src0));
            }
        } break;
        case GGML_OP_REPEAT: {
            if (src0_needs_grads) {
        ...
        case GGML_OP_CONCAT: {
            ...
            if (src0_needs_grads) {
                struct ggml_tensor * back = ggml_view_4d(ctx, grad,
                        src0->ne[0], src0->ne[1], src0->ne[2], src0->ne[3],
                        grad->nb[1], grad->nb[2], grad->nb[3], 0);
                ggml_add_or_set(ctx, cgraph, isrc0, ggml_cont(ctx, back));
            }
            if (src1_needs_grads) {
                ...
                ggml_add_or_set(ctx, cgraph, isrc1, ggml_cont(ctx, back));
            }
        } break;
        case GGML_OP_RMS_NORM: {
            if (src0_needs_grads) {
                ...
                struct ggml_tensor * rms_back = ctx->fused_backward && grad->op == GGML_OP_MUL && grad->src[1] &&
                        grad->src[1]->type == GGML_TYPE_F32 && grad->src[1]->ne[0] == src0->ne[0] &&
                        ggml_nrows(grad->src[1]) == 1
                    ? ggml_rms_norm_mul_back(ctx, grad->src[0], src0, grad->src[1], eps)
                    : ggml_rms_norm_back(ctx, grad, src0, eps);
                ggml_add_or_set(ctx, cgraph, isrc0, rms_back);
            }
        } break;
        ...
        case GGML_OP_SOLVE_TRI: {
            if (src0_needs_grads || src1_needs_grads) {
                const int64_t n = src0->ne[0];
                struct ggml_tensor * identity = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, n);
                identity = ggml_diag(ctx, ggml_fill(ctx, identity, 1.0f));
                if (!ggml_are_same_shape(identity, src0)) {
                    identity = ggml_repeat(ctx, identity, src0);
                }

                struct ggml_tensor * a_inv = ggml_solve_tri(ctx, src0, ggml_cont(ctx, identity), true, true, false);
                struct ggml_tensor * a_inv_t = ggml_cont(ctx, ggml_transpose(ctx, a_inv));
                struct ggml_tensor * grad_t = ggml_cont(ctx, ggml_transpose(ctx, grad));
                struct ggml_tensor * grad_b = ggml_cont(ctx, ggml_transpose(ctx, ggml_mul_mat(ctx, a_inv_t, grad_t)));

                if (src0_needs_grads) {
                    struct ggml_tensor * grad_a = ggml_neg(ctx, ggml_mul_mat(ctx, tensor, grad_b));
                    grad_a = ggml_tri(ctx, ggml_cont(ctx, grad_a), GGML_TRI_TYPE_LOWER_DIAG);
        ...
        case GGML_OP_SCALE: {
            if (src0_needs_grads) {
                ...
                struct ggml_tensor * grad_scale = ggml_is_padded_1d(grad) ? grad : ggml_cont(ctx, grad);
                ggml_add_or_set(ctx, cgraph, isrc0, ggml_scale_impl(ctx, grad_scale, s, 0.0, false));
            }
        } break;
        case GGML_OP_SET: {
        ...
        case GGML_OP_PAD: {
            ...
            if (src0_needs_grads) {
                ...
                const size_t offset = lp0*grad->nb[0] + lp1*grad->nb[1] + lp2*grad->nb[2] + lp3*grad->nb[3];
                struct ggml_tensor * back = ggml_view_4d(ctx, grad,
                        src0->ne[0], src0->ne[1], src0->ne[2], src0->ne[3],
                        grad->nb[1], grad->nb[2], grad->nb[3], offset);
                ggml_add_or_set(ctx, cgraph, isrc0, ggml_cont(ctx, back));
        ...
        case GGML_OP_TRI: {
            if (src0_needs_grads) {
                const enum ggml_tri_type type = ggml_get_op_params_i32(tensor, 0);
                ggml_add_or_set(ctx, cgraph, isrc0, ggml_tri(ctx, ggml_cont(ctx, grad), type));
            }
        } break;
        case GGML_OP_FILL: {
            // The output does not depend on the input values.
        } break;
        case GGML_OP_CPY: {
        ...
        case GGML_OP_DIAG: {
            if (src0_needs_grads) {
                struct ggml_tensor * ones = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, src0->ne[0]);
                struct ggml_tensor * identity = ggml_diag(ctx, ggml_fill(ctx, ones, 1.0f));
                struct ggml_tensor * back = ggml_sum_rows(ctx, ggml_mul(ctx, grad, identity));
                ggml_add_or_set(ctx, cgraph, isrc0, ggml_reshape(ctx, back, src0));
            }
        } break;
        ...
        case GGML_OP_CROSS_ENTROPY_LOSS: {
            if (src0_needs_grads) {
                struct ggml_tensor * loss_back = src1->type == GGML_TYPE_I32
                    ? ggml_cross_entropy_loss_sparse_back(ctx, grad, src0, src1, src2)
                    : ggml_cross_entropy_loss_back(ctx, grad, src0, src1);
                ggml_add_or_set(ctx, cgraph, isrc0, loss_back);
            }
            GGML_ASSERT(!src1_needs_grads && !src2_needs_grads && "backward pass for labels not implemented");
        } break;
        case GGML_OP_GLU: {
            switch (ggml_get_glu_op(tensor)) {
                case GGML_GLU_OP_SWIGLU: {
                    GGML_ASSERT(src1 && "backward pass only implemented for split swiglu");
                    if (ctx->fused_backward && src0_needs_grads && src1_needs_grads) {
                        struct ggml_tensor * back = ggml_glu_back(ctx, grad, src0, src1, GGML_GLU_OP_SWIGLU);
                        if (src0_needs_grads) {
                            ggml_add_or_set(ctx, cgraph, isrc0, ggml_view_4d(ctx, back,
                                src0->ne[0], src0->ne[1], src0->ne[2], src0->ne[3],
                                back->nb[1], back->nb[2], back->nb[3], 0));
                        }
                        if (src1_needs_grads) {
                            ggml_add_or_set(ctx, cgraph, isrc1, ggml_view_4d(ctx, back,
                                src1->ne[0], src1->ne[1], src1->ne[2], src1->ne[3],
                                back->nb[1], back->nb[2], back->nb[3], src0->ne[0]*sizeof(float)));
                        }
                    } else {
                        if (src0_needs_grads) {
                ...
                case GGML_GLU_OP_GEGLU: {
                    ...
                    if (ctx->fused_backward && src0_needs_grads && src1_needs_grads) {
                        struct ggml_tensor * back = ggml_glu_back(ctx, grad, src0, src1, GGML_GLU_OP_GEGLU);
                        if (src0_needs_grads) {
                            ggml_add_or_set(ctx, cgraph, isrc0, ggml_view_4d(ctx, back,
                                src0->ne[0], src0->ne[1], src0->ne[2], src0->ne[3],
                                back->nb[1], back->nb[2], back->nb[3], 0));
