...
TYPES_KV_EXT = ["GGML_TYPE_F16", "GGML_TYPE_MXFP4", "GGML_TYPE_TURBO3_0", "GGML_TYPE_TURBO4_0"]
...
SOURCE_FATTN_VEC_EXT = """#include "../fattn-vec.cuh"
...
for type_k in TYPES_KV_EXT:
    for type_v in TYPES_KV_EXT:
        if type_k == "GGML_TYPE_F16" and type_v == "GGML_TYPE_F16":
            continue
        with open(f"fattn-vec-instance-{get_short_name(type_k)}-{get_short_name(type_v)}.cu", "w") as f:
            f.write(SOURCE_FATTN_VEC_EXT.format(type_k=type_k, type_v=type_v))
        ...
        with open(f"fattn-mma-f16-instance-ncols1_{ncols1}-ncols2_{ncols2}.cu", "w") as f:
            ...
            for head_size_kq in HEAD_SIZES_KQ:
                ...
                if head_size_kq == 320 and ncols2 != 32: # Mistral Small 4
                    continue
                if head_size_kq == 512: # Gemma 4 (+ MTP)
                    if ncols2 not in (2, 4, 8) and not (ncols1 == 1 and ncols2 == 32):
                        continue
                if head_size_kq == 576 and ncols2 not in (4, 16, 32): # Deepseek, GLM 4.7 Flash
                    continue
                if head_size_kq not in (192, 320, 512, 576) and ncols2 in (16, 32) and not (head_size_kq == 256 and ncols1 == 1 and ncols2 == 32):
                    continue
                head_size_v = HEAD_SIZES_V_OVERRIDE.get(head_size_kq, head_size_kq)
